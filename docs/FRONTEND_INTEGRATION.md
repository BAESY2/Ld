...
### P-C. WebGL 이식 — 🚧 2단계: 플랫폼 통합 완료 — `frontend/twin3d.js`(프리팹 lib+라인 빌더 3종: 포장 컨베이어·AGV 섹터·용접 로봇셀), 렌더러 토글(지연 로드·라인 전환 재마운트·미지원 라인 캔버스 자동 폴백), PLC 상태 실시간 바인딩(벨트·부품·AGV 위치/비콘·로봇 관절·스파크 Points). **10/10 라인 빌더 완료**(대차·세차·배치 포함 — 액체 레벨·교반 샤프트 회전·아치 emissive·캐리지). 남음: WebGL 기기 클릭/전체 라벨·디테일 고도화(GLTF)
