...
**헤더 20바이트 = BE, 명령블록 = LE** (가장 흔한 구현 버그 — 테스트로 못박을 것).

| 오프셋 | 크기 | 필드 | 값 |
|---|---|---|---|
| 0 | 8 | Company ID | `"LSIS-XGT"` |
| 13 | 1 | Source of Frame | 0x33(요청)/0x11(응답) |
| 14 | 2 | Invoke ID(BE) | 응답 echo(txn id) |
| 16 | 2 | Length(BE) | 명령블록 바이트수 |
...
**엔디언 함정:** MELSEC 전부 LE / FEnet 헤더 BE·블록 LE — 라운드트립 목테스트로 못박기.
