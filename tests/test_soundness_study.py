...
인터락 쌍을 실행오라클(시뮬레이터)이 단 한 번도 반증하지 못하며(false-proof=0), 오라클이
찾은 위반은 형식검증도 빠짐없이 탐지한다(miss=0). 무작위 시드 고정 → 결정론.
전체(trials=400, random=300)는 스크립트로 돌리고, 여기선 빠른 표본만 검사한다.
"""
