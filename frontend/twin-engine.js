...
const P=(x,y,z)=>[view.ox+(x-y)*0.866*view.s, view.oy+(x+y)*0.5*view.s-(z||0)*0.8*view.s];
...
weld:{
 ...
 tick(st,o,dt){
   ...
   st.arm=ramp(st.arm,o.WELD?1:0,2.2,dt);
   ...
   if(o.WELD){st.weaveT+=dt;
     const tip=P(7.55+Math.sin(st.weaveT*9)*0.07,2.28,1.02);
 ...
 draw(ctx,st,val,t){
   ...
   dq(6.3+2.95+1.4,c2=>{
     ...
     const T=a<0.02?P(6.85,2.95,2.0):P(6.85+( (7.55+wv) -6.85)*a,2.95+(2.3-2.95)*a,2.0+(1.04-2.0)*a);
...
function pickDevice(e){
  ...
  if(hit){devOpen=hit.tg;
    ...
    el.style.left=Math.max(6,Math.min(e.clientX-r.left+14,cvs.clientWidth-248))+"px";
    el.style.top=Math.max(6,Math.min(e.clientY-r.top+6,cvs.clientHeight-160))+"px";
    renderDev();
  } else hideDev();
}
function renderDev(){
  ...
  if(ent){
    ...
    h+=`<div class="dvr"><span>설비</span><b>${info.name}</b></div>`+
     ...
     `<div class="dvr"><span>상태</span><b style="color:${tripped?"#f85149":on?"#3fb950":"#8b98a8"}">${tripped?"TRIP — 복구 필요":on?"RUN":"STOP"}</b></div>`+
     `<div class="dvr"><span>전류 / 정격</span><b>${amp} A / ${info.rated} A</b></div>`+
     `<div class="dvr"><span>가동 · 기동</span><b>${stt.rt.toFixed(0)}s · ${stt.starts}회</b></div>`;
  }else{
    h+=`<div class="dvr"><span>유형</span><b>${(chip&&chip.sub)||"계측·구조물"}</b></div>`+
     `<div class="dvr"><span>제어</span><b>PLC 비구동(수동/센서)</b></div>`;
  }
  h+=`<button class="xbtn" onclick="hideDev()">닫기</button>`;
  el.innerHTML=h;
}
...
function makeLine(id){
  ...
  const L={id,d,m,plc:makePLC(d.ladder),scene:SCENES[m.scene],scanN:0,us:0,lastKey:"",stats:{},
    ...
    speedF:1,ngRate:8,demandF:1,faults:{},auto:true,apPulse:null,apCool:Math.random()*2,cpu:"XGK-CPUH"};
  L.sst=L.scene.init();
