...
(function(){
  ...
  const fb=document.createElement("button");
  fb.className="xbtn";fb.id="factbtn";fb.textContent="🏭 전체 공장";
  fb.title="가동 전 라인을 하나의 대형 공장으로 — 실시간 동기";
  document.querySelector(".scol").insertBefore(fb,sb);
  fb.onclick=()=>{FACT.on?exitFactory():enterFactory();};
  /* 래더 탭 → 엔지니어링 SW 진입 */
...
function openPlcTool(){
  ...
  const upd=()=>{
    ...
    const rows=[];
    cur.d.sim.inputs.forEach(s=>rows.push([cur.m.addr[s]||s,s,cur.plc.val(s)?"1":"0",cur.plc.val(s)]));
    cur.d.sim.outputs.forEach(s=>rows.push([cur.m.addr[s]||s,s,eff(s)?"1":"0",eff(s)]));
    for(const[s,t2]of Object.entries(cur.plc.timers))rows.push([cur.m.addr[s]||s,s,t2.acc.toFixed(1)+"/"+t2.preset+"s",t2.q]);
    for(const[s,c2]of Object.entries(cur.plc.counters))rows.push([cur.m.addr[s]||s,s,c2.cnt+"/"+c2.preset,c2.q]);
    for(const[addr,nm,get]of dRegs(cur))rows.push([addr,nm,String(get()),false]);
    document.getElementById("pt_dev").innerHTML=rows.map(([a,s,v,on])=>
     `<tr><td><b>${a}</b></td><td class="mut">${s}</td><td style="text-align:right;color:${on?"#7ee787":"#8b98a8"};font-weight:700">${v}</td></tr>`).join("");
