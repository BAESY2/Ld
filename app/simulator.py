...
# 아날로그 비교 원자(boolexpr 가 'SIG op NUM' 을 Var("SIG>=5") 로 파싱). op 은 길이순
# (>=,<=,<> 먼저)으로 매칭해야 ">" 가 ">=" 를 가로채지 않는다. boolexpr._CMP_OPS 와 동일 집합.
_CMP_ATOM_RE = re.compile(
...
class _Compare:
    """파싱된 비교 원자: signal op threshold (수치 신호값으로 평가)."""
...
def _parse_compare(name: str) -> _Compare | None:
    """Var 이름이 비교 원자('SIG>=5')면 (신호, op, 임계) 로 분해, 아니면 None.

    boolexpr.parse 가 'SIG op NUM' 을 공백 없는 단일 Var 이름으로 합성하므로
    여기서도 공백 없는 형태를 가정한다(예: 'PRESSURE>=5').
    """
    m = _CMP_ATOM_RE.match(name)
    if not m:
        return None
    return _Compare(signal=m.group(1), op=m.group(2), threshold=float(m.group(3)))


def _eval_compare(cmp_: _Compare, table: Mapping[str, bool | float]) -> bool:
    """비교 원자를 수치 신호값으로 평가한다(미지정 신호는 0 으로 간주).

    불리언 입력이 비교 신호로 쓰이면 True→1.0 / False→0.0 으로 환산한다.
    """
    lhs = float(table.get(cmp_.signal, False))
    rhs = cmp_.threshold
    if cmp_.op == ">=":
        return lhs >= rhs
    if cmp_.op == "<=":
...
def _eval(node: Node, table: Mapping[str, bool | float]) -> bool:
    match node:
        case Var(name):
            cmp_ = _parse_compare(name)
            if cmp_ is not None:
                return _eval_compare(cmp_, table)
...
class _Timer:
    ...
    def scan(self, table: Mapping[str, bool | float], dt: int) -> None:
...
class _Counter:
    ...
    def scan(self, table: Mapping[str, bool | float]) -> None:
...
class SimSample:
    ...
    # 아날로그 비교에 쓰인 수치 신호값(예: {"PRESSURE": 6.0}). 불리언 전용 가동에선 비어 있다.
...
class SimResult:
    ...
    # 비교기에 쓰인 아날로그 신호 심볼(없으면 빈 리스트 → 하위호환).
    signals: list[str] = field(default_factory=list)

    def output_trace(self, sym: str) -> list[bool]:
        return [s.outputs.get(sym, False) for s in self.samples]

    def signal_trace(self, sym: str) -> list[float]:
        """수치 신호의 시간축 값 트레이스(아날로그 입력 확인용)."""
        return [s.signals.get(sym, 0.0) for s in self.samples]
...
class _Program:
    ...
    def numeric_signals(self) -> set[str]:
        """비교 원자(예: 'PRESSURE>=5')가 참조하는 아날로그 신호 심볼 집합."""
        out: set[str] = set()
        for sym in self.symbols():
            cmp_ = _parse_compare(sym)
            if cmp_ is not None:
                out.add(cmp_.signal)
        return out
...
def simulate(
    st_code: str,
    inputs_timeline: Sequence[tuple[int, Mapping[str, bool | float]]],
    *,
    duration_ms: int,
    step_ms: int = 100,
) -> SimResult:
    ...
    값은 불리언(BOOL 입력) 또는 수치(int/float, 아날로그 신호)다. 수치 신호는 합성된
    비교기('SIG op NUM' 원자)의 평가에 쓰여 비교 플래그를 켠다. 기존 불리언 전용 호출은
    100% 그대로 동작한다(수치 신호가 없으면 수치 경로가 비활성).
    ...
    table: dict[str, bool | float] = {}
    driven = prog.driven
    fb_q = {f"{n}.Q" for n in prog.timers} | {f"{n}.Q" for n in prog.counters}
    all_syms = prog.symbols()
    signal_syms = prog.numeric_signals()
    # 비교 원자(예: 'PRESSURE>=5')는 사용자 입력 심볼이 아니므로 제외하고, 그 안의
    # 아날로그 신호(PRESSURE)는 수치 입력으로 따로 노출한다.
    inputs = sorted(
        s for s in all_syms
        if s not in driven and s not in fb_q and "." not in s
        and s not in signal_syms and _parse_compare(s) is None
    )
    signals = sorted(signal_syms)
    for s in driven:
        table[s] = False

    timeline = sorted(inputs_timeline, key=lambda x: x[0])
    cur_bool: dict[str, bool] = {s: False for s in inputs}
    cur_sig: dict[str, float] = {s: 0.0 for s in signals}
    samples: list[SimSample] = []
    ti = 0
    t = 0
    while t <= duration_ms:
        # 1) 입력 읽기 (해당 시각까지의 입력 변화 반영) — 불리언/수치 분리 저장.
        while ti < len(timeline) and timeline[ti][0] <= t:
            for sym, val in timeline[ti][1].items():
                if sym in cur_sig and not isinstance(val, bool):
                    cur_sig[sym] = float(val)
                else:
                    cur_bool[sym] = bool(val)
            ti += 1
        for s in inputs:
            table[s] = cur_bool.get(s, False)
        for s in signals:
            table[s] = cur_sig.get(s, 0.0)
        ...
        samples.append(SimSample(
            t_ms=t,
            inputs={s: bool(table.get(s, False)) for s in inputs},
            outputs={s: bool(table.get(s, False)) for s in driven},
            signals={s: float(table.get(s, 0.0)) for s in signals},
        ))
        t += step_ms
