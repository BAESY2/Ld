...
if TYPE_CHECKING:  # 순환 임포트 방지 — 런타임 임포트는 build_entries() 안에서.
...
def build_entries() -> list[ErrorCode]:
    """지식베이스 항목 생성 — error_codes 와의 순환 임포트를 끊는 지연 팩토리.

    (error_codes 가 모듈 말미에서 이 함수를 호출해 DB 를 구성한다. 어느 쪽을
    먼저 import 해도 안전하다 — 직접 `import app.error_kb` 가 첫 임포트여도
    모듈 토플레벨에서는 error_codes 를 건드리지 않는다.)
    """
    from app.error_codes import ErrorCode, Vendor  # noqa: PLC0415
    ...
    return [
        ...
        _e(vendor=Vendor.LS_ELECTRIC, series="XGT 공통", code="ERR-LED-BLINK",
           ...
           likely_cause="운전은 지속되지만 주의가 필요한 상태(배터리 저하·퓨즈 단선·"
           "일부 모듈 경고).",
           suggested_action="운전 중이라도 방치 금지. 에러 이력에서 경고 종류 확인 → "
           "배터리/퓨즈류는 "
        ...
        _e(vendor=Vendor.LS_ELECTRIC, series="XGT 공통", code="BAT-LED",
           ...
           likely_cause="CPU 백업 배터리(리튬) 수명 종료 임박 — 정전 시 래치 데이터·시계 "
           "소실 위험.",
        ...
        _e(vendor=Vendor.LS_ELECTRIC, series="XGT 공통", code="IO_DETACH",
           ...
           category="IO", severity="FATAL",
           likely_cause="운전 중 I/O 모듈이 빠졌거나(진동·베이스 록 풀림) 접점 산화로 "
           "착탈로 오인식.",
        ...
        _e(vendor=Vendor.SIEMENS, series="S7-1200/1500", code="ARRAY-IDX-OOB",
           ...
           source_url=_SRC_S7_CYCLE_DB,
           source_doc="IndustrialMonitorDirect KB — DB 액세스 오류 해결",
...
def __getattr__(name: str) -> object:
    """하위호환 — `from app.error_kb import KB_ENTRIES` 를 지연 평가로 지원."""
    if name == "KB_ENTRIES":
